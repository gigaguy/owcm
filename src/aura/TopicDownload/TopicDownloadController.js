({
    ...
    selectAll: function(component, event, helper) { 
        ...
        if(isSelected){
           ...
           //added by kingro to fix selectAll bug
                   component.set('v.selectedTopicIds',allMatchedRecordIds);
    ...
    //Mounika Suram 8/8/2018 Updated to handle the checkAll snippets by passing selectAll parameter instead of snippetids
    downloadAction: function(component, event, helper) {
        
      console.log("in downloadAction");
        ...
        console.log("strExpOpt" + strExpOpt);
        var checkBoxAll = document.getElementById('selectAll');
        console.log("checkBoxAll" + checkBoxAll);
        var selectedRecords = component.get('v.selectedTopicIds');
        // holds the topicid of thr present record
        var topicId = component.get('v.recordId');
        var selectedRecords = component.get('v.selectedTopicIds');
        console.log("selectedRecordscount" + selectedRecords.length);
        var allRecords= component.get('v.allSnippets');
        console.log("allRecordscount" + allRecords.length);
        var strtopicId= topicId.toString;
        // Roy King - Added filterText==null so selectAll=true isnt passed in the url
//        if(checkBoxAll.checked && (selectedRecords.length==allRecords.length) && filterText == null) {
        if(checkBoxAll.checked ) {
            var urlEvent = $A.get("e.force:navigateToURL");
            var url = "/apex/TopicDownloadPage?selectAll=true";
            url = url + "&topicId="+topicId;
            url = url + "&expOptions="+strExpOpt;
            // OWCM-109 - added searchString
            if (filterText != null){
                url = url + "&searchString="+filterText;
                console.log('searchString - ' + filterText);
            }
            console.log('url='+url);
            urlEvent.setParams({
                "url": url,
        ...
        else if(checkBoxAll.checked && filterText.length > 0) {
            var urlEvent = $A.get("e.force:navigateToURL");
            var url = "/apex/TopicDownloadPage?selectAll=true";
            url = url + "&topicId="+topicId;
            url = url + "&expOptions="+strExpOpt;
